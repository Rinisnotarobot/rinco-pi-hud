...
import { parseMcpStatus } from "../extensions/hud/segments/mcp-status.ts";
import { __test__ as packageVersionTest } from "../extensions/hud/segments/package-version.ts";
import {
	formatSkillCounts,
	SkillActivityTracker,
} from "../extensions/hud/segments/skill-activity.ts";
...
test("config counts tolerate missing files and use a custom agent directory", async () => {
...
test("config counts isolate corrupt settings from other counts", async () => {
...
test("config counts call the agent directory provider when no explicit directory is supplied", async () => {
