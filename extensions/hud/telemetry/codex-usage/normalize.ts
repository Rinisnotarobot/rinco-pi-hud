...
export function normalizeAppServerResponse(
	...
	const addSnapshot = (raw: unknown, fallbackId: string, optional = false) => {
		...
		const existingIndex = snapshots.findIndex((item) => item.limitId === resolvedSnapshot.limitId);
		const existing = existingIndex >= 0 ? snapshots[existingIndex] : undefined;
		if (existingIndex >= 0 && existing)
			snapshots[existingIndex] = mergeSnapshot(existing, resolvedSnapshot);
		else snapshots.push(resolvedSnapshot);
	};
