...
/**
 * Look up the sanitized `target` (e.g. file path) recorded for a tool call at
 * `tool_execution_start` time. `ToolExecutionEndEvent` does not carry `args`,
 * so callers that need the original arguments (e.g. to detect which file a
 * `read` call touched) must recover them from telemetry state instead of the
 * end event.
 */
export function findToolTarget(state: TelemetryState, toolCallId: string): string | undefined {
	const sanitizedId = sanitizeExternalText(toolCallId, 256);
	for (let index = state.tools.length - 1; index >= 0; index -= 1) {
		const tool = state.tools[index];
		if (tool?.toolCallId === sanitizedId) return tool.target;
	}
	return undefined;
}
