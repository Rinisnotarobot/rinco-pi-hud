import type { GitReadResult } from "./git.js";
import { emptyGitStatus } from "./git.js";
import type { PackageVersionReadResult } from "./package-version.js";
import type { RuntimeReadResult } from "./runtime.js";
 ...
 *
 * Deliberately mutates `state` in place rather than returning a new object:
 * this runs on every project-refresh tick of a long-lived render loop, and
 * `FooterState` is a single shared, non-reactive mutable record read
 * directly by the footer's `render()` — allocating a fresh object per tick
 * would add churn with no observable benefit. This is the one place in the
 * codebase that intentionally diverges from the immutable-update convention
 * used elsewhere (e.g. `state/telemetry.ts`).
 *
