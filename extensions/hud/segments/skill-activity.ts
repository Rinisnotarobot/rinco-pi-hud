...
/**
 * Normalize a skill path to a canonical absolute form for equality checks.
 *
 * Scope note: the result is only ever used as a `Map`/`Set` key to compare
 * "available skill" paths against "activated skill" paths — it is never
 * passed to `fs`/`child_process`. It intentionally does not verify the
 * normalized path stays under `cwd`; if a future change starts using this
 * path for file I/O, add that containment check first.
 */
