/**
 * Static runtime/toolchain detection table (Starship `runtime`-module parity).
 *
 * Pulled out of `runtime.ts` because it is >90% data: one entry per
 * language/build-system with its detection rules and version-probe command(s).
 * Detection matching, caching, and the public `detectRuntime`/`readRuntimeInfo`
 * API live in `runtime.ts`; this module only describes *what* each runtime looks
 * like and *how* to ask its toolchain for a version string.
 */

import { execFile } from "node:child_process";
import { promisify } from "node:util";
import type { DetectionSpec, RuntimeMetadata } from "./runtime.js";
...
/** Runtimes sorted by priority — stable sort preserves definition order within the same priority. */
export const runtimeDefinitions: readonly RuntimeDef[] = [...runtimes].sort(
